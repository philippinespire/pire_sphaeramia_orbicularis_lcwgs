...
# This script will generate the historical_samples.txt config file for GenErode.
# It should be run in the species' config GenErode directory. The historical directory should have all of the historical (Albatross) *.fq.gz files.
# example: /archive/carpenterlab/pire/pire_lethrinus_variegatus_lcwgs/GenErode_Lva_20k/historical
...
base_path=$"/archive/carpenterlab/pire/pire_${species}_lcwgs/GenErode_${Spp}_20k_3seqruns"

# Define the input files
input_file="${base_path}/config/old_new_lane_GenErode_${Spp}_config.log"

# Define the output files
output_file_historical="${base_path}/config/historical_samples.txt"
error_file_historical="${base_path}/config/error_historical.out"

# Create or clear the output files and add the header line
echo "samplename_index_lane readgroup_id readgroup_platform path_to_R1_fastq_file path_to_R2_fastq_file" > "$output_file_historical"

# Loop over each *1.fq.gz file in the historical directory
for r1file in ../historical/*1.fq.gz; do
    ...
    if [ -e "$r1file" ]; then
        ...
        if [ -e "../historical/$r2file_name" ]; then
            ...
            if [ "$r1_index" != "$r2_index" ]; then
                echo "Warning: Index mismatch for $samplename illumina historical/$r1file_name historical/$r2file_name" >> "$error_file_historical"
                continue
            fi
            index="$r1_index"
			...
			if [ -n "$r1_lane" ] && [ -n "$r2_lane" ]; then
				if [ "$r1_lane" != "$r2_lane" ]; then
					echo "Warning: Lane mismatch for $samplename illumina historical/$r1file_name historical/$r2file_name" >> "$error_file_historical"
					continue
				fi
				lane="$r1_lane"
			else
				echo "Warning: Lane information missing for $samplename illumina historical/$r1file_name historical/$r2file_name" >> "$error_file_historical"
			fi
            ...
            # Output to historical_samples.txt with only
            echo "$samplename_index_lane $readgroup_id $readgroup_platform historical/$r1file_name historical/$r2file_name" >> "$output_file_historical"
        else
            echo "Warning: Corresponding R2 file for $r1file_name not found." >> "$error_file_historical"
        fi
    fi
done

# Completion message
echo "Historical samples processing completed. Output saved to historical_samples.txt"

# Count the lines in the output file excluding the header
count_output=$(($(wc -l < "$output_file_historical") - 1))

# Count the number of R1 files
count_r1=$(find ../historical/ -name "*1.fq.gz" | wc -l)

# Count the number of R2 files
count_r2=$(find ../historical/ -name "*2.fq.gz" | wc -l)

# Verify if all counts match
if [ "$count_output" -eq "$count_r1" ] && [ "$count_output" -eq "$count_r2" ]; then
    echo "All $count_r1 1.fq.gz and $count_r2 2.fq.gz files were incorporated into historical_samples.txt"
else
    echo "Mismatch detected:"
    echo "- Lines in historical_samples.txt (excluding header): $count_output"
    echo "- R1 files found: $count_r1"
    echo "- R2 files found: $count_r2"
    echo "Check error_historical.out for details."
fi
